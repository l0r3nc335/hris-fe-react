...
### Install shadcn primitives before LoginPage or Suspense fallbacks.

> **Note:** Auth forms and route loading depend on this step. Do not skip to §6 without `button`, `input`, `label`, `form`, `card`, and `skeleton`.

## Minimum for auth + routing

Install via shadcn CLI ([`components.json`](components.json), `radix-nova` style):

`button`, `input`, `label`, `form`, `card`, `skeleton`
